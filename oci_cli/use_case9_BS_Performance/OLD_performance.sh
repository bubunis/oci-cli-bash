...
oci compute boot-volume-attachment list --availability-domain $AVAIL_DOM -c $COMP_OCID_INSTANCE |jq -C -r '.data[]|"\(."boot-volume-id") \(.id)"'|grep $OCID_INSTANCE|awk '{print $1}'


green_color "MOUNTED DISKS"
green_color "-------------"
...
echo -e "\n"
read -p "CHOOSE THE DISK WHERE YOU ARE FACING ISSUE: " DISK
	...
	1)
		...
		if [ $? -gt 0 ];then
			...
			echo -e "CAN'T RUN fio.WRONG $MOUNT." >> $BS_OPT		
                	...
                	fio --filename=$MOUNT/file --size=1GB --direct=1 --rw=randrw --bs=4k --ioengine=libaio --iodepth=256 --runtime=20 --numjobs=4 --time_based --group_reporting --name=iops-test-job 1>>output.txt &
			PID=$!
			F_PROGRESS_STAT $PID
		fi
